...
def load_dataset(uni_folder):
    ...
    task=['task01_camelyon','task02_Her2_region','task03_Gland','task04_PanNuke','task05_ConSep','task06_lizard','task07_HP','task08_Ki67','task09_MBM','task10_Her2','task11_nuclei']
...
class DataLoader(SlimDataLoaderBase):
    def __init__(self, data, batch_size, number_of_threads_in_multithreaded=None,task_num=11,train_dataset_len=[],transforms=None):
        ...
        self.dataset=data
        self.list_of_keys=['task01_camelyon','task02_Her2_region','task03_Gland','task04_PanNuke','task05_ConSep','task06_lizard','task07_HP','task08_Ki67','task09_MBM','task10_Her2','task11_nuclei']
...
data_generator=DataLoader(train_uni_dataset,batch_size=1,train_dataset_len=train_dataset_len,transforms=transforms)
...
# model_path='/media/ipmi2022/Elements/backup/xuzhengyang/code/universal_model/universal_segmantation/unet_time_newdata_2024-11-04_18_27_53_epoch_950.pkl'
...
for epoch in tqdm(range(1,epochs),desc='Epoch'):
    ...
    # print(epoch)
    # optimizer.zero_grad()
    for it in tqdm(range(30*11)):#一共10个数据集，每个挑30个
...
#         
        ...
        # print(loss)
        # # print(out.shape)
    mean_loss=mean_loss/330
    mean_ce=mean_ce/330
    mean_dice=mean_dice/330

    tqdm.write(f"Epoch {epoch}: Loss={mean_loss:.4f}, cross entropy={mean_ce:.4f}, dice={mean_dice:.4f}")
    with open('/media/ipmi2022/Elements/backup/xuzhengyang/code/universal_model/universal_segmantation/loss.txt', 'a') as log_file:
